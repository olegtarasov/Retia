...
namespace Retia.Tests.Mathematics
{
    ...
    public class GpuAlgorithmsTests
    {
        ...
        {
            var m1 = MatrixFactory.RandomMatrix<float>(100, 100, 1e-5f, 20.0f);
            var m2 = MatrixFactory.RandomMatrix<float>(100, 100, 1e-5f, 20.0f);
            ...
            using (var matrixPtrs = new HostMatrixPointers<float>(m1.CloneMatrix(), m2.CloneMatrix()))
            {
                ...
                double gpu = GpuInterface.Testing.TestCrossEntropyError(matrixPtrs.Definitions[0], matrixPtrs.Definitions[1]);

                double.IsNaN(cpu).ShouldBeFalse();
                double.IsNaN(gpu).ShouldBeFalse();

                gpu.ShouldEqualWithinError(cpu, 1e-4f);
            }
        }
